...
export default function App() {
  return (
    <BrowserRouter>
      ...
      <div id="app-root">
        <Routes>
          {/* Public routes — login + first time org gate. */}
          <Route path="/" element={<Navigate to="/login" replace />} />
          <Route path="/login" element={<LoginScreen />} />
          <Route path="/org/setup" element={<OrgGateScreen />} />

          {/* Every other route lives behind the auth gate. The gate
              enforces (session AND at least one membership); a missing
              session bounces to /login, a missing membership bounces to
              /org/setup. Design data still lives in localStorage in
              Phase 1A — the gate is a routing layer only. */}
